...
| Branch | issue-267-m116-uuid |
| Next step | **M117 LAST_INSERT_ID(expr)** (#268). Label `agent-ready` after M116 merge. Phase R issues #265–#283 filed. Phases S–Z filed (#285–#363, milestones 10–17) but not `agent-ready`. Ultimate MySQL 8.0 goal is **not** complete. |
...
| Layer | Status |
|-------|--------|
| CI on `main` | Green (PR #365 M115) |
| Roadmap M36–M61 + PERF-B* | Complete |
| Phase Q (M62–M113) | **Complete** — last merge M113 PR #263 |
| Phase R (M114–M132) | **M114–M115 on main** (PRs #346 / #365); **M116 in this PR** — GitHub milestone [Phase R](https://github.com/tanbamboo/rusql/milestone/9); next `agent-ready` [#268 M117](https://github.com/tanbamboo/rusql/issues/268) |
...
Gap probe `scripts/mysql-gap-probe.mjs` on `main` after M113: **29 probes, 19 rusql gaps**, 9 ok, 1 both-fail (`CREATE PROCEDURE … IN` / `DELIMITER`). M114 charset DDL, M115 `JSON_EXTRACT($.key)`, and M116 `UUID()` close three of those gaps once this PR merges.

Session exit check (Docker `mysql:8.0` client → rusql): session introspection OK (Phase Q exit).

Remaining probe gaps now have issues: charset DDL (#265, **done** PR #346), JSON_EXTRACT (#266, **done** PR #365), UUID (#267, this PR), LAST_INSERT_ID(expr) (#268), GET_LOCK (#269), TABLE_CONSTRAINTS (#270), PROCESSLIST I_S (#271), PARAMETERS (#272), SHOW BINARY LOGS/EVENTS (#273/#274), OR REPLACE VIEW (#275), text PREPARE (#276), SAVEPOINT (#277), WITH RECURSIVE (#278), INTERSECT (#279), window frames (#280), DISABLE ON SLAVE (#281), SHOW ENGINE INNODB STATUS (#282), procedure IN (#283). Later stages S–Z are filed (#285–#363): JSON pack, schema, locking, programs, replication, TLS, observability, remaining engine — not `agent-ready`. Ultimate goal still unmet.

## Recent Progress

- **M116** — `UUID()` RFC 4122 v4 hyphenated hex form; extra args i18n arity error; mysql-diff `compare_output: false` (#267)
