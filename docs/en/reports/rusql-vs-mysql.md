...
**As of:** 2026-09-20 (`main`, after M109)  
...
| Can a typical production schema + ORM migrate unchanged? | **No** — gaps such as `TRUNCATE`, `REPLACE`, `INSERT IGNORE`, JSON extract, text `PREPARE` |
...
| Schema (`CREATE`/`ALTER`/`INDEX`/`FK`) | Common patterns work; no `TRUNCATE` | Dev / subset only |
...
|-------|------------------|-------------------|------|
| **`mysql-diff`** | Same SQL on rusql **and** Docker MySQL 8.0 via official `mysql` CLI | **297 steps**, 55 suites + 2 protocol-smoke queries (290 unique texts) | **CI** — last run **297/297** |
...
Of the 297 `mysql-diff` steps, **77** run on both servers but skip row-text equality (`compare_output: false`) — typically `SHOW` / version / metadata that is allowed to differ.
...
| Need `TRUNCATE` / `REPLACE` / `INSERT IGNORE` / JSON extract / advisory locks | **Not yet** (backlog) |
