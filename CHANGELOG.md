...
- **M118** — `SELECT GET_LOCK('gap_lock', 0)` returns `1` when the name is free and `0` when another connection holds it (timeout `0` is non-blocking). `SELECT RELEASE_LOCK('gap_lock')` returns `1` on the holder, `0` if someone else holds it, and NULL if the name is not locked. The same session re-`GET_LOCK`s a name it already holds as `1`. Disconnect, `COM_RESET_CONNECTION`, and `COM_CHANGE_USER` release that session's locks. `timeout > 0` does not wait (returns `0` immediately if held; waiting is M164). Not InnoDB row locks / `FOR UPDATE` wait (M157) and not `IS_FREE_LOCK` / `IS_USED_LOCK`. M117 `LAST_INSERT_ID(expr)`, M116 `UUID()`, and M115 `JSON_EXTRACT` unchanged. mysql-diff suite `get_lock` compares single-connection GET then RELEASE (#269).
- **M117** — `SELECT LAST_INSERT_ID(expr)` returns the coerced value and sets that connection's session `last_insert_id` so a following `SELECT LAST_INSERT_ID()` returns the same number. Coercion matches MySQL's unsigned integer conversion on portable cases: nearest integer, half away from zero (`LAST_INSERT_ID(5.9)` → `6`), non-numeric → `0`, negatives wrap as `BIGINT UNSIGNED`. Other connections are unchanged. `COM_RESET_CONNECTION` / `COM_CHANGE_USER` still clear the value (M75). Generated `AUTO_INCREMENT` INSERT ids still overwrite it; AUTO_INCREMENT allocation is unchanged. The setter updates `session.last_insert_id`, so later DML OK packets report that value until a generated INSERT overwrites it. Extra arguments are an i18n arity error. Not `GET_LOCK`. M75 no-arg `LAST_INSERT_ID()`, M116 `UUID()`, and M115 `JSON_EXTRACT` unchanged. mysql-diff suite `last_insert_id` compares `LAST_INSERT_ID(5)` then `LAST_INSERT_ID()` (#268).
- **M116** — `SELECT UUID()` returns one RFC 4122 version 4 identifier in MySQL's 8-4-4-4-12 hyphenated hex form (36 characters). Two calls on the same connection differ. Extra arguments are an i18n arity error. rusql uses random v4, not MySQL's time-based v1, so `mysql-diff` suite `uuid` uses `compare_output: false`. Not `UUID_TO_BIN` / `BIN_TO_UUID` / `UUID_SHORT` / `SYS_GUID`. M115 `JSON_EXTRACT` and M113 builtins unchanged (#267).
- **M115** — `JSON_EXTRACT(json, path)` evaluates the `$.key` / `$.a.b` object-path subset. `SELECT JSON_EXTRACT('{"a":1}', '$.a')` returns MySQL 8.0's unquoted `1`. A missing path is SQL NULL (empty cell), not an error. Invalid JSON is errno 3141 (`ER_INVALID_JSON_TEXT`) with an i18n message. Not `JSON_SET`, `->` / `->>`, `JSON_OBJECT`, `JSON_TABLE`, or full JSONPath. M113 builtins unchanged. mysql-diff suite `json_extract` (#266).
...
- **M113** — `SUBSTRING`/`SUBSTR` (MySQL 1-based `pos`; optional `len`; negative `pos` counts from the end), `ROUND`/`ROUND(x, d)` (half away from zero, not banker's rounding; parsed as `f64`), and `DATE_ADD`/`ADDDATE` with `INTERVAL n {SECOND|MINUTE|HOUR|DAY|WEEK|MONTH|YEAR}`. Date-only input plus `DAY`/`WEEK`/`MONTH`/`YEAR` returns `YYYY-MM-DD`; otherwise `YYYY-MM-DD HH:MM:SS`. `MONTH`/`YEAR` reuse the M105 30/365-day approximation. Unknown other functions stay unsupported. M46 builtins unchanged. mysql-diff suite `substring_round_date_add` (#255).
- **M112** — `INSERT IGNORE INTO t VALUES (…)` skips PRIMARY KEY conflicts (`affected_rows` is the number of rows actually inserted; the existing row is unchanged). A new PK inserts as usual. Multi-row `INSERT IGNORE` inserts non-conflicting rows and skips duplicates. Plain `INSERT` duplicate PK stays errno 1062. M68 `ON DUPLICATE KEY UPDATE` and M111 `REPLACE INTO` are unchanged. Not UNIQUE-not-PK, not sql_mode truncation IGNORE, and not `SHOW WARNINGS` notes. mysql-diff suite `insert_ignore` (#254).
- **M111** — `REPLACE INTO t VALUES (…)` inserts when the PRIMARY KEY is new (`affected_rows` 1) and delete-then-inserts on a single-column PK conflict (`affected_rows` 2; old row gone, new values stored). Reuses `AFTER DELETE` then `BEFORE INSERT` trigger hooks. Composite PRIMARY KEY is rejected (same single-column limit as M68 ODKU). Plain `INSERT` duplicate PK stays errno 1062. `INSERT … ON DUPLICATE KEY UPDATE` is unchanged. `INSERT IGNORE` stays unsupported. mysql-diff suite `replace_into` (#253).
- **M110** — `TRUNCATE TABLE t` and `TRUNCATE t` empty a base table (heap `delete_rows` with no filter) and reset `AUTO_INCREMENT` to 1 when present (`set_auto_increment` + session catalog). OK `affected_rows` is 0. Unknown tables are errno 1146; `information_schema` / `__rusql_*` virtual tables and views are rejected. Does not fire `AFTER DELETE` triggers. Not partition/`CASCADE` truncate and not InnoDB tablespace reuse. `DELETE` / `DROP TABLE` unchanged. mysql-diff suite `truncate_table` (#252).
