...
**MySQL 8.0 CLI** (Issue #73): when `CLIENT_QUERY_ATTRIBUTES` is negotiated, clients prepend a WL#12542 attribute block before the SQL string on `COM_QUERY`; rusql strips it before parsing. Resultsets end with an OK packet (not legacy EOF) when `CLIENT_DEPRECATE_EOF` is negotiated, with a metadata EOF/OK between column definitions and rows. OK packets include a session-state trailer when `CLIENT_SESSION_TRACK` is negotiated.
