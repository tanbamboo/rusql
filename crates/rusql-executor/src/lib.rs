            ...
            if let SetExpr::Select(select) = query.body.as_ref() {
                if let Some(from) = select.from.first() {
                    ...
                    if let TableFactor::Table { name, .. } = &from.relation {
                        ...
                        if table == show_create_event::CREATE_EVENT_VIRTUAL_TABLE {
                            let eqs = parse_where_filter(select.selection.as_ref())
                                .ok()
                                .flatten()
                                .map(|f| eq_prefix_from_filter(&f))
                            ...
                            let name = eqs
                                ...
                                .map(|(_, v)| v.as_str())
                                .ok_or_else(|| {
                                    ExecError::Message("SHOW CREATE EVENT requires an event".into())
                                })?;
                            let database = eqs
                                .iter()
                        ...
                        if table == show_create_trigger::CREATE_TRIGGER_VIRTUAL_TABLE {
                            let eqs = parse_where_filter(select.selection.as_ref())
                                .ok()
                                .flatten()
                                .map(|f| eq_prefix_from_filter(&f))
                            ...
                            let name = eqs
                                ...
                                .ok_or_else(|| {
                                    ExecError::Message(
                                        "SHOW CREATE TRIGGER requires a trigger".into(),
                                    )
                                })?;
                            let database = eqs
                                .iter()
                                .find(|(col, _)| col == "__db__")
                                .map(|(_, v)| v.as_str());
                            return show_create_trigger::show_create_trigger(
                                session, database, name,
                            );
                        }
                        if table == show_create_procedure::CREATE_PROCEDURE_VIRTUAL_TABLE {
                            let eqs = parse_where_filter(select.selection.as_ref())
                                .ok()
                                .flatten()
                                .map(|f| eq_prefix_from_filter(&f))
                            ...
                            let name = eqs
                                ...
                                .ok_or_else(|| {
                                    ExecError::Message(
                                        "SHOW CREATE PROCEDURE requires a procedure".into(),
                                    )
                                })?;
                            let database = eqs
                                .iter()
                        ...
                        if table == show_procedure_status::PROCEDURE_STATUS_VIRTUAL_TABLE {
                            let eqs = parse_where_filter(select.selection.as_ref())
                                .ok()
                                .flatten()
                                .map(|f| eq_prefix_from_filter(&f))
                        ...
                        let table_columns: Vec<String> = table_meta
                            ...
                            .unwrap_or_default();
                        let table_collations = table_column_collations(session, &table);
    ...
    for opt in &c.options {
        match &opt.option {
            ...
            ColumnOption::CharacterSet(name) => {
                col.collation = Collation::from_name(&object_name_to_string(name));
            }
            ColumnOption::DialectSpecific(tokens) => {
                col.collation = collation_from_dialect_tokens(tokens);
            }
            _ => {}
        }
    }
...
fn collation_from_dialect_tokens(tokens: &[sqlparser::tokenizer::Token]) -> Option<Collation> {
    let joined = tokens
        .iter()
        .map(|t| t.to_string())
        .collect::<Vec<_>>()
    ...
    Collation::supported()
        ...
        .find(|c| {
            joined
                .to_ascii_lowercase()
                .contains(&c.name().to_ascii_lowercase())
        })
        .copied()
}
...
fn collations_for_output_columns(
...
fn table_column_collations(session: &Session, table: &str) -> Vec<Collation> {
    session
        .catalog
        .get_table(table)
        .map(|m| m.columns.iter().map(|c| c.effective_collation()).collect())
        .unwrap_or_default()
}
...
fn apply_order_by(mut rows: Vec<Row>, keys: &[SortKey]) -> Vec<Row> {
    ...
    rows.sort_by(|a, b| {
        for key in keys {
            ...
            let ord = key.collation.compare(va, vb);
...
mod tests {
    ...
    #[test]
    fn show_collation_lists_supported_collations() {
        let mut session = Session::new(1, "root");
        let mut exec = heap_executor();
        ...
        match &results[0] {
            QueryResult::Rows { columns, rows } => {
                ...
                let ai_row = rows.iter().find(|r| r[0] == "utf8mb4_0900_ai_ci").unwrap();
                assert_eq!(ai_row[3], "");
            }
            _ => panic!("expected rows"),
        }
    ...
    fn show_create_trigger_rows(
        ...
        match results.into_iter().next().unwrap() {
            QueryResult::Rows { columns, rows } => (columns, rows),
            other => panic!("expected SHOW CREATE TRIGGER rows for {sql}, got {other:?}"),
        }
    }

    #[test]
    fn show_create_trigger_from_catalog_and_neighbors_unchanged() {
        use rusql_core::{TriggerEvent, TriggerMeta, TriggerTiming, DEFAULT_SCHEMA};
        ...
        let (_, after_rows) =
            show_create_trigger_rows(&mut exec, &mut session, "SHOW CREATE TRIGGER `tr_after`");
        assert_eq!(
            after_rows[0][2],
            "CREATE TRIGGER `tr_after` AFTER UPDATE ON `src` FOR EACH ROW INSERT INTO src VALUES (OLD.id)"
        );

        let (_, qualified) =
            show_create_trigger_rows(&mut exec, &mut session, "SHOW CREATE TRIGGER rusql.tr_src");
        assert_eq!(qualified, rows);

        let plans = plan(
            &session,
            parse("SHOW CREATE TRIGGER no_such_trigger").unwrap(),
        );
        match exec.execute(&mut session, &plans, None) {
            Err(ExecError::Mysql { code, message }) => {
                assert_eq!(code, 1360);
        ...
        let (trig_cols, trig_rows) = show_triggers_rows(&mut exec, &mut session, "SHOW TRIGGERS");
        assert_eq!(
            trig_cols,
            show_triggers::COLUMNS
                .iter()
        ...
        assert_eq!(trig_rows.len(), 2);
        let src = trig_rows.iter().find(|r| r[0] == "tr_src").unwrap();
        assert_eq!(src[1], "INSERT");
        assert_eq!(src[3], "SET NEW.id = NEW.id");
        assert_eq!(src[7], "root@%");
    ...
    fn show_create_procedure_rows(
        ...
        match results.into_iter().next().unwrap() {
            QueryResult::Rows { columns, rows } => (columns, rows),
            other => panic!("expected SHOW CREATE PROCEDURE rows for {sql}, got {other:?}"),
        }
    }

    #[test]
    fn show_create_procedure_from_catalog_and_neighbors_unchanged() {
        ...
        let (_, qualified) =
            show_create_procedure_rows(&mut exec, &mut session, "SHOW CREATE PROCEDURE rusql.p");
        assert_eq!(qualified, rows);

        let plans = plan(
            &session,
            parse("SHOW CREATE PROCEDURE no_such_proc").unwrap(),
        );
        match exec.execute(&mut session, &plans, None) {
            Err(ExecError::Mysql { code, message }) => {
                assert_eq!(code, 1305);
    ...
    fn show_procedure_status_rows(
        ...
        match results.into_iter().next().unwrap() {
            QueryResult::Rows { columns, rows } => (columns, rows),
            other => panic!("expected SHOW PROCEDURE STATUS rows for {sql}, got {other:?}"),
        }
    }

    #[test]
    fn show_procedure_status_from_catalog_and_neighbors_unchanged() {
        use rusql_core::{
            FunctionMeta, ProcedureMeta, TriggerEvent, TriggerMeta, TriggerTiming, DEFAULT_SCHEMA,
        };
        ...
        let (_, like_rows) =
            show_procedure_status_rows(&mut exec, &mut session, "SHOW PROCEDURE STATUS LIKE 'p%'");
        assert_eq!(like_rows, rows);

        let (_, empty) = show_procedure_status_rows(
            &mut exec,
            &mut session,
            "SHOW PROCEDURE STATUS LIKE 'no_such%'",
        );
    ...
    #[test]
    fn show_create_event_missing_and_neighbors_unchanged() {
        use rusql_core::{
            Account, FunctionMeta, ProcedureMeta, AUTH_PLUGIN_NATIVE, DEFAULT_SCHEMA,
        };
        ...
        let plans = plan(
            &session,
            parse("SHOW CREATE EVENT rusql.`no_such`").unwrap(),
        );
        match exec.execute(&mut session, &plans, Some(&store)) {
            Err(ExecError::Mysql { code, message }) => {
                assert_eq!(code, 1539);
    ...
    #[test]
    fn collation_0900_order_by_and_where_eq() {
        use rusql_core::corpus::utf8mb4_0900_ai_ci::{COLLATION, EQUAL_PAIRS, SORT_ASC};

        let mut session = Session::new(1, "root");
        let mut exec = heap_executor();
        let plans = plan(
            &session,
            parse("CREATE TABLE names (id INT, name VARCHAR(64) COLLATE utf8mb4_0900_ai_ci)")
                .unwrap(),
        );
        exec.execute(&mut session, &plans, None).unwrap();
        ...
        match &results[0] {
            QueryResult::Rows { rows, .. } => {
                ...
                for w in got.windows(2) {
                    assert!(
                        COLLATION.compare(w[0], w[1]).is_le(),
                        "not sorted: {:?} vs {:?}",
                        w[0],
                        w[1]
                    );
        ...
        let plans = plan(
            &session2,
            parse("CREATE TABLE eq_t (id INT, name VARCHAR(64) COLLATE utf8mb4_0900_ai_ci)")
                .unwrap(),
        );
        exec2.execute(&mut session2, &plans, None).unwrap();
        for (i, (stored, _)) in EQUAL_PAIRS.iter().enumerate() {
