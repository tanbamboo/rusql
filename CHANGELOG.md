...
- **M117** — `SELECT LAST_INSERT_ID(expr)` returns the coerced value and sets that connection's session `last_insert_id` so a following `SELECT LAST_INSERT_ID()` returns the same number. Coercion matches MySQL's unsigned integer conversion on portable cases: truncate toward zero (`LAST_INSERT_ID(5.9)` → `5`), non-numeric → `0`, negatives wrap as `BIGINT UNSIGNED`. Other connections are unchanged. `COM_RESET_CONNECTION` / `COM_CHANGE_USER` still clear the value (M75). Generated `AUTO_INCREMENT` INSERT ids still overwrite it; AUTO_INCREMENT allocation is unchanged. The setter updates `session.last_insert_id`, so later DML OK packets report that value until a generated INSERT overwrites it. Extra arguments are an i18n arity error. Not `GET_LOCK`. M75 no-arg `LAST_INSERT_ID()`, M116 `UUID()`, and M115 `JSON_EXTRACT` unchanged. mysql-diff suite `last_insert_id` compares `LAST_INSERT_ID(5)` then `LAST_INSERT_ID()` (#268).
