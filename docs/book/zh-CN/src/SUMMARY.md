# 目录

[引言](./introduction.md)

---

# 第零篇 — 背景

- [MySQL 兼容全景](./part0/mysql-landscape.md)

---

# 第一篇 — Harness Engineering

- [为何采用 Harness Engineering？](./part1/why-harness.md)
- [rusql 的 Harness 技术栈](./part1/harness-stack.md)
- [面向 Agent 的项目建议](./part1/harness-tips.md)

---

# 第二篇 — 里程碑

- [M0 — Harness 引导](./part2/m00-harness-bootstrap.md)
- [M1 — 线缆协议握手](./part2/m01-wire-handshake.md)
- [M2 — COM_QUERY](./part2/m02-com-query.md)
- [M3 — WAL 持久化](./part2/m03-wal-persistence.md)
- [M4 — 二级索引](./part2/m04-indexes.md)
- [M5 — 兼容性夹具套件](./part2/m05-compat-fixtures.md)
- [M6 — 认证与 DML](./part2/m06-auth-dml.md)
- [M7 — caching_sha2_password](./part2/m07-caching-sha2.md)
- [M8 — UPDATE](./part2/m08-update.md)
- [M9 — 事务](./part2/m09-transactions.md)
- [M10 — SHOW TABLES](./part2/m10-show-tables.md)
- [M11 — 预编译语句](./part2/m11-prepared-statements.md)
- [M12 — DESCRIBE 与 information_schema](./part2/m12-describe-info-schema.md)
- [M13 — SHOW CREATE TABLE](./part2/m13-show-create-table.md)
- [M14 — SELECT 列投影](./part2/m14-select-projection.md)
- [M15 — USE database](./part2/m15-use-database.md)
- [M16 — SELECT LIMIT](./part2/m16-select-limit.md)
- [M17 — SELECT ORDER BY](./part2/m17-order-by.md)
- [M18 — SELECT 列别名](./part2/m18-column-aliases.md)
- [M19 — SELECT LIMIT OFFSET](./part2/m19-limit-offset.md)
- [M20 — WHERE 比较与 AND](./part2/m20-where-comparisons.md)
- [M21 — IS NULL / IS NOT NULL](./part2/m21-is-null.md)
- [M22 — INNER JOIN](./part2/m22-inner-join.md)
- [M23 — PRIMARY KEY 元数据](./part2/m23-primary-key-metadata.md)
- [M24 — ALTER TABLE ADD COLUMN](./part2/m24-alter-add-column.md)
- [M25 — 二进制结果集](./part2/m25-binary-resultset.md)
- [M26 — caching_sha2 RSA 完整认证](./part2/m26-caching-sha2-rsa.md)

---

# 附录

- [指标快照](./appendix/metrics.md)
- [参考书目](./appendix/bibliography.md)
- [延伸阅读](./appendix/further-reading.md)
