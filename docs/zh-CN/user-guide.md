...
**结论（2026-09-20）：** rusql **不能**作为 MySQL 8.0 的生产即插即用替代。官方 `mysql` CLI 在不断扩展的 SQL 子集上可用。实时对比为相对 Docker MySQL 8.0 的 **322/322** 条 `mysql-diff` 步骤。
...
| REPLACE INTO | 完成 | M111 主键冲突先删后插；`affected_rows` 为 1（插入）或 2（替换）；仍不支持 `INSERT IGNORE` |
...
`REPLACE INTO t VALUES (…)` 在主键为新值时插入（`affected_rows` 为 1）。单列主键冲突时先删除旧行再插入新值（`affected_rows` 为 2，旧行消失）。复合主键会被拒绝（与 M68 相同的单列限制）。普通 `INSERT` 重复主键仍为 errno 1062。`INSERT … ON DUPLICATE KEY UPDATE` 行为不变。仍不支持 `INSERT IGNORE`。不实现多表 REPLACE，也不处理非主键 UNIQUE 冲突。
